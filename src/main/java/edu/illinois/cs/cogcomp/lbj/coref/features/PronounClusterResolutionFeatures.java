 ...
 */
public class PronounClusterResolutionFeatures {
	...
	protected PronounClusterResolutionFeatures() {
	}
	...
	public static boolean matchesRelative(String w) {
		if (w.equals("who") || w.equals("whom") || w.equals("which")
				...
				|| w.equals("whichever") || w.equals("that"))
			return true;
		else
			return false;
	}
	...
	public static String rightPOS(Mention m) {
		if (m.getExtentFirstWordNum() == m.getDoc().getWords().size() - 1)
			return "None";
		else
	...
	public static String aroundPOS(Mention m) {
		if (m.getExtentFirstWordNum() == m.getDoc().getWords().size() - 1 || m.getExtentFirstWordNum() == 0)
			return "None";
		else
	...
	public static String pronounCompatibility(CExample ex) {
		...
		if (!(proEntityType.equals(Constants.Unknown) || proEntityType
				.equals("PER"))
				&& PronounClusterResolutionFeatures.isFirstorSecondPerson(pro))
			return "f"; // 0,0
		if (proEntityType.equals("PER")
				&& PronounClusterResolutionFeatures.proIsHuman(ex) == 'N')
			return "f"; // 0, 0
		if (proEntityType.equals("PER")
