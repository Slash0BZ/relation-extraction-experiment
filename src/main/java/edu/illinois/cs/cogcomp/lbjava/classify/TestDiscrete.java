 ...
 *     java TestDiscrete [-t &lt;n&gt;] &lt;classifier&gt;
 *                                     &lt;oracle&gt; &lt;parser&gt;
...
public class TestDiscrete {
    ...
    public static TestDiscrete testDiscrete(TestDiscrete tester, Classifier classifier,
        ...
        if (output && outputGranularity > 0) {
            ...
            for (example = parser.next(); example != null; example = parser.next(), ++processed) {
                ...
                if (r.getAttribute("RelationSubtype") == "Employment"){
                    System.out.println("Source Type: " + r.getSource().getAttribute("EntityType"));
                    System.out.println("Target Type: " + r.getTarget().getAttribute("EntityType"));
                }
            ...
            for (example = parser.next(); example != null; example = parser.next(), ++processed) {
                /*
                Relation r = Relation.class.cast(example);
                if (r.getAttribute("RelationSubtype") == "Employment"){
                    System.out.println("Source Type: " + r.getSource().getAttribute("EntityType"));
                    System.out.println("Target Type: " + r.getTarget().getAttribute("EntityType"));
                }
    ...
    private static TestDiscrete instantiate(String[] args) {
        ...
        try {
            ...
            System.err
                    .println("usage:\n"
                            + "  java TestDiscrete [-t <n>] <classifier> <oracle> \\\n"
                            + "                                  <parser> <input file> \\\n"
    ...
    public void printPerformance(PrintStream out) {
        ...
        Double total_labeled = 0.0;
        Double total_correct = 0.0;
        Double total_none_null_correct = 0.0;
        Double total_none_null_labeled = 0.0;
        Double total_none_null_predicted = 0.0;
        for (int i = 0; i < allClasses.length; ++i) {
            int correct = getCorrect(allClasses[i]);
            int predicted = getPredicted(allClasses[i]);
            int labeled = getLabeled(allClasses[i]);
            if (allClasses[i].equals("NOT_RELATED") == false){
                total_none_null_correct += correct;
                total_none_null_labeled += labeled;
                total_none_null_predicted += predicted;
            }
            total_predicted += (double)predicted;
            total_labeled += (double)labeled;
            total_correct += (double)correct;
            ...
            if (correct > 0) {
                ...
                totalF1 = totalF1 + table[i][2];
                totalPre = totalPre + table[i][0];
                totalRec = totalRec + table[i][1];
                totalCount ++;
            }
        }
        System.out.println("Total Predicted: " + total_predicted);
        System.out.println("Total Labeled: " + total_labeled);
        double avgPre = totalPre / totalCount;
        double avgRec = totalRec / totalCount;
        System.out.println("Maro Precision: " + avgPre);
        System.out.println("Macro Recall: " + avgRec);
        System.out.println("Macro F1: " + (2 * avgPre * avgRec) / (avgPre + avgRec));
        double mAvgPre = total_none_null_correct / total_none_null_predicted;
        double mAvgRec = total_none_null_correct / total_none_null_labeled;
        System.out.println("Micro Precision: " + mAvgPre);
        System.out.println("Micro Recall: " + mAvgRec);
        System.out.println("Micro F1: " + (2 * mAvgPre * mAvgRec) / (mAvgPre + mAvgRec));
