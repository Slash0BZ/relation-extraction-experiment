...
public class FileBackedDictionary implements DictionaryDatabase {
   ...
   **/
	...
	protected Cache<DatabaseKey, Object> entityCache =
    new LRUCache<DatabaseKey, Object>(DEFAULT_CACHE_CAPACITY);
	...
	protected class DatabaseKey {
		...
		DatabaseKey(POS pos, Object key, boolean forSynset) {
			this.pos = pos;
			this.key = key;
			this.forSynset = forSynset;
		}
		...
		public boolean equals(Object object) {
			return object instanceof DatabaseKey
				...
				&& ((DatabaseKey) object).forSynset == forSynset;
		}

		public int hashCode() {
			return 31 * pos.hashCode() + 17 * key.hashCode() + (forSynset ? 7 : 0);
		}
	}
	...
	protected IndexWord getIndexWordAt(POS pos, long offset) {
    	DatabaseKey key = new DatabaseKey(pos, new Long(offset), false);
	...
	protected Synset getSynsetAt(POS pos, long offset, String line) {
    	DatabaseKey key = new DatabaseKey(pos, new Long(offset), true);
    ...
    public IndexWord lookupIndexWord(POS pos, String string) {
    	DatabaseKey key = new DatabaseKey(pos, string, false);
		IndexWord word = (IndexWord) entityCache.get(key);
		if (word == null) {
			String filename = getIndexFilename(pos);
			long offset;
